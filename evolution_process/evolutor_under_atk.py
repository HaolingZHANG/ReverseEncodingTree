...
# parser.add_argument("--Atktype",  default = str, 'gaussian-avg', help= "choose attack type")
# parser.add_argument("--noiselev", type = float, default= 0.1, help= "ratio of random time frame select" )
# args = parser.parse_args()

obs_atker = atk_model( Ftype = 'gaussian-avg') # args.Atktype
noise_level = 0.1 # agrs.noiselev
    ...
    def _genome_in_reinforced(self, genome, config):
        ...
        for episode in range(self.episode_generation):
            ...
            for step in range(self.episode_steps):
                ...
                if done:
                    print("With: ", round(num_atk_recv*100/(float(step)+1),2),"% Attack", end = "\n")
                    break
                else:
